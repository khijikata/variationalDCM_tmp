...
mc_dina_data_gen <- function(I = 500, Q ,match_p=0.80,not_match="equal", att_cor = 0, att_threshold = "rand",seed = 1234){
  ...
  if( not_match == "equal"){
    ...
    for(j in 1:J){
      ...
      match_p_temp <- runif(dim_G_mat[1], min=0.5, max=1.0)
      i_par_temp  <- diag(match_p_temp)
      i_par_temp[lower.tri(i_par_temp)] <- runif(sum(lower.tri(i_par_temp)), min=0, max=0.5)
      i_par_temp[upper.tri(i_par_temp)] <- runif(sum(upper.tri(i_par_temp)), min=0, max=0.5)
      i_par_temp[,ncol(i_par_temp)] <- runif(nrow(i_par_temp),min=0.2,max=0.8)
...
#'   \item{a_0}{the estimates of Dirichlet distribution's parameters, which is the prior for \theta}
...
mcdina = function(X,Q,max_it  = 500,
  ...
  for(m in 1:max_it){
    ...
    for (l in temp_order_l) {
      ...
      temp_max <- optimise(f, maximum = T,lower = 10E-5,upper = 10E5)$maximum
